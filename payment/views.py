...
def start_stripe(request, order_id):
    if request.method == "GET":
        ...
        booking = get_object_or_404(Booking, order_id=order_id)
        context = {"booking": booking}
        return render(request, "payment/stripe_payment_create.html", context=context)
            ...
            line_items=[
                {
                    "price_data": {
                        ...
                        "product_data": {
                            "name": "Cameo from {} {}".format(
                                cameo.first_name, cameo.last_name
                            ),
                            "images": [
...
def stripe_success(request):
    ...
    booking = get_object_or_404(Booking, order_id=request.session.get("transaction_id"))
